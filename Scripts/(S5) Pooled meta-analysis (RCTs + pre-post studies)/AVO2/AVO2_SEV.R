...
AVO2SEV <- read.xlsx("AVO2_DATA_mixedout.xlsx", sheet = 3)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2SEV, ylim = c(-1,45), xlim = c(-11.5,7), alim = c(-2,2), rows = c(40:14, 9:2), cex=0.4,
...
addpoly(COMP_REM_AVO2SEV, row=12.5, cex=0.4, col=11, mlab="")
text(-8, 12.5, cex=0.4, bquote(paste("RE Model for Motor-Complete: p < 0.002",
                                     "; ", I^2, " = ", .(formatC(COMP_REM_AVO2SEV$I2, digits = 1, format="f")), "%")))

addpoly(INCOMP_REM_AVO2SEV, row=0.5, cex=0.4, col=11, mlab="")
text(-8.05, 0.7, cex=0.4, bquote(paste("RE Model for Motor-Incomplete: p = 0.08",
                                        "; ", I^2, " = ", .(formatC(INCOMP_REM_AVO2SEV$I2, digits = 1, format="f")), "%")))

text(c(-7, -6, -5, -4, -3), 44, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-6.5, -4.5), 45, c('Pre', 'Post'), cex=0.3)
text(3.9, 44, cex=0.4, bquote(paste(bold("Weight"))))
...
text(-9.4, 41.5, cex=0.44, bquote(paste(bolditalic("Motor-Complete (AIS A-B)"))))
text(-9.35, 10.5, cex=0.44, bquote(paste(bolditalic("Motor-Incomplete (AIS C-D)"))))

text(-7.75, -2, cex=0.4, "Test for Subgroup Differences: Q = 2.90, df = 1, p = 0.09")
...
legend("topright", c("Motor-complete","Motor-incomplete"),cex=0.8, pch=20, col=c("purple","blue"))
