...
AVO2CLASS <- read.xlsx("AVO2_DATA_mixedout.xlsx", sheet = 2)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2CLASS, ylim = c(-1,35), xlim = c(-11.5,7), alim = c(-2,2), rows = c(30:29, 24:2), cex = 0.4,
...
addpoly(TETRA_REM_AVO2CLASS, row=27.5, cex=0.4, col=11, mlab="")
text(-8.4, 27.5, cex=0.4, bquote(paste("RE Model for Tetraplegia: p = 0.23",
                                       "; ", I^2, " = ", .(formatC(TETRA_REM_AVO2CLASS$I2, digits = 1, format="f")), "%")))

addpoly(PARA_REM_AVO2CLASS, row=0.5, cex=0.4, col=11, mlab="")
text(-8.35, 0.7, cex=0.4, bquote(paste("RE Model for Paraplegia: p < 0.002",
                                        "; ", I^2, " = ", .(formatC(PARA_REM_AVO2CLASS$I2, digits = 1, format="f")), "%")))

text(c(-7, -6, -5, -4, -3), 34, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-6.5, -4.5), 35, c('Pre', 'Post'), cex=0.3)
text(3.9, 934, cex=0.4, bquote(paste(bold("Weight"))))
...
text(-10.45, 31.5, cex=0.44, bquote(paste(bolditalic("Tetraplegia"))))
text(-10.45, 25.5, cex=0.44, bquote(paste(bolditalic("Paraplegia"))))

text(-7.75, -2, cex=0.4, "Test for Subgroup Differences: Q = 0.31, df = 1, p = 0.58")
