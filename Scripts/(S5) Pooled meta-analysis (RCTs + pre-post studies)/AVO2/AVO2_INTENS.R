...
AVO2INTENS <- read.xlsx("AVO2_DATA_mixedout.xlsx", sheet = 5)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2INTENS, ylim = c(-1,66), xlim = c(-11.5,7), alim = c(-2,2), rows = c(61:61, 56:49, 44:21, 16:2), cex = 0.4,
...
addpoly(LIGHT_REM_AVO2INTENS, row=59.5, cex=0.4, col=11, mlab="")
text(-8.8, 59.5, cex=0.4, bquote(paste("RE Model for Light: p = 0.85",
                                        "; ", I^2, " = ", .(formatC(LIGHT_REM_AVO2INTENS$I2, digits = 1, format="f")), "%")))

addpoly(MOD_REM_AVO2INTENS, row=47.5, cex=0.4, col=11, mlab="")
text(-8.5, 47.5, cex=0.4, bquote(paste("RE Model for Moderate: p = 0.01",
                                        "; ", I^2, " = ", .(formatC(MOD_REM_AVO2INTENS$I2, digits = 1, format="f")), "%")))

addpoly(MODVIG_REM_AVO2INTENS, row=19.5, cex=0.4, col=11, mlab="")
text(-7.62, 19.5, cex=0.4, bquote(paste("RE Model for Moderate-to-Vigorous: p < 0.003",
                                        "; ", I^2, " = ", .(formatC(MODVIG_REM_AVO2INTENS$I2, digits = 1, format="f")), "%")))

addpoly(VIG_REM_AVO2INTENS, row=0.5, cex=0.4, col=11, mlab="")
text(-8.52, 0.7, cex=0.4, bquote(paste("RE Model for Vigorous: p < 0.003",
                                        "; ", I^2, " = ", .(formatC(VIG_REM_AVO2INTENS$I2, digits = 1, format="f")), "%")))


text(c(-7, -6, -5, -4, -3), 65, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-6.5, -4.5), 66, c('Pre', 'Post'), cex=0.3)
text(3.9, 65, cex=0.4, bquote(paste(bold("Weight"))))
...
text(-10.75, 62.5, cex=0.44, bquote(paste(bolditalic("Light"))))
text(-10.5, 57.5, cex=0.44, bquote(paste(bolditalic("Moderate"))))
text(-9.65, 45.5, cex=0.44, bquote(paste(bolditalic("Moderate-to-Vigorous"))))
text(-10.57, 17.5, cex=0.44, bquote(paste(bolditalic("Vigorous"))))

text(-7.75, -2, cex=0.4, "Test for Subgroup Differences: Q = 2.00, df = 3, p = 0.57")
...
legend("topright", c("Light","Moderate","Mod-to-Vig","Vigorous"),cex=0.7, pch=20, col=c("purple","blue","orange","red"))
