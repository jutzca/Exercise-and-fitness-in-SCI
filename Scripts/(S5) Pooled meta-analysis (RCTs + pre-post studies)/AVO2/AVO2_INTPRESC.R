...
AVO2INTPRESC <- read.xlsx("AVO2_DATA_mixedout.xlsx", sheet = 6)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2INTPRESC, ylim = c(-1,64), xlim = c(-11.5,7), alim = c(-2,2), rows = c(59:52, 47:30, 25:16, 11:2), cex = 0.4,
...
addpoly(VO2_REM_AVO2INTPRESC, row=50.5, cex=0.4, col=11, mlab="")
text(-7.7, 50.5, cex=0.4, bquote(paste("RE Model for Oxygen Consumption: p = 0.003",
                                       "; ", I^2, " = ", .(formatC(VO2_REM_AVO2INTPRESC$I2, digits = 1, format="f")), "%")))

addpoly(HR_REM_AVO2INTPRESC, row=28.5, cex=0.4, col=11, mlab="")
text(-8.33, 28.5, cex=0.4, bquote(paste("RE Model for Heart Rate: p < 0.002",
                                       "; ", I^2, " = ", .(formatC(HR_REM_AVO2INTPRESC$I2, digits = 1, format="f")), "%")))

addpoly(RPE_REM_AVO2INTPRESC, row=14.5, cex=0.4, col=11, mlab="")
text(-8.8, 14.5, cex=0.4, bquote(paste("RE Model for RPE: p < 0.002",
                                       "; ", I^2, " = ", .(formatC(RPE_REM_AVO2INTPRESC$I2, digits = 1, format="f")), "%")))

addpoly(WLOAD_REM_AVO2INTPRESC, row=0.5, cex=0.4, col=11, mlab="")
text(-8.48, 0.7, cex=0.4, bquote(paste("RE Model for Workload: p < 0.002",
                                       "; ", I^2, " = ", .(formatC(WLOAD_REM_AVO2INTPRESC$I2, digits = 1, format="f")), "%")))

text(c(-7, -6, -5, -4, -3), 63, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-6.5, -4.5), 64, c('Pre', 'Post'), cex=0.3)
text(3.9, 63, cex=0.4, bquote(paste(bold("Weight"))))
...
text(-9.75, 60.5, cex=0.44, bquote(paste(bolditalic("Oxygen Consumption"))))
text(-10.45, 48.5, cex=0.44, bquote(paste(bolditalic("Heart Rate"))))
text(-9.3, 26.5, cex=0.44, bquote(paste(bolditalic("Rating of Percieved Exertion"))))
text(-10.55, 12.5, cex=0.44, bquote(paste(bolditalic("Workload"))))

text(-7.75, -2, cex=0.4, "Test for Subgroup Differences: Q = 1.86, df = 4, p = 0.60")
...
my_colours <- c("purple","blue","orange","red")[match(AVO2INTPRESC$subgroup,c("vo2", "hr", "rpe", "wload"))]
...
legend("topright", c("Oxygen consumption","Heart rate","RPE","Workload"),cex=0.6, pch=20, col=c("purple","blue","orange","red"))
