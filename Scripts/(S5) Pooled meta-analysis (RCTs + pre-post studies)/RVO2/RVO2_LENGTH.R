...
RVO2LENGTH <- read.xlsx("RVO2_DATA_mixedout.xlsx", sheet = 7)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_RVO2LENGTH, ylim = c(-1, 92), xlim = c(-85, 60), at = c(-10, 0, 10, 20), rows = c(87:65, 60:22, 17:2), digits = 1, cex=0.4,
...
addpoly(SIX_REM_RVO2LENGTH, row=63.5, cex=0.4, col=11, mlab="")
text(-61, 63.5, cex=0.38, bquote(paste("RE Model for "<="6 weeks",
                                         ": p < 0.001; ", I^2, " = ", .(formatC(SIX_REM_RVO2LENGTH$I2, digits = 1, format="f")), "%")))

addpoly(SIXTWELVE_REM_RVO2LENGTH, row=20.5, cex=0.4, col=11, mlab="")
text(-56.6, 20.5, cex=0.38, bquote(paste("RE Model for > 6 to "<="12 weeks",
...
text(-77.5, 88.5, cex=0.4, (bquote(paste(bolditalic(" "<= "6 weeks")))))
text(-73, 61.5, cex=0.4, (bquote(paste(bolditalic("> 6 to "<="12 weeks")))))
text(-76, 18.5, cex=0.4, bquote(paste(bolditalic("> 12 weeks"))))

text(-55.7, -2, cex=0.38, "Test for Subgroup Differences: Q = 8.28, df = 2, p = 0.016")
