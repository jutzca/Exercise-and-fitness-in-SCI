...
PPOLENGTH <- read.xlsx("PPO_DATA_mixedout.xlsx", sheet = 7)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_PPOLENGTH, ylim = c(-1, 79), xlim = c(-180, 135), at = c(-40, -20, 0, 20, 40, 60), rows = c(74:57, 52:29, 24:2), digits = c(0,0), cex=0.5,
...
addpoly(SIX_REM_PPOLENGTH, row=55.5, cex=0.5, col=11, mlab="")
text(-117, 55.5, cex=0.5, bquote(paste("RE Model for "<="6 weeks",
                                       ": p < 0.001; ", I^2, " = ", .(formatC(SIX_REM_PPOLENGTH$I2, digits = 1, format="f")), "%")))

addpoly(SIXTWELVE_REM_PPOLENGTH, row=27.5, cex=0.5, col=11, mlab="")
text(-107.9, 27.5, cex=0.5, bquote(paste("RE Model for > 6 to "<="12 weeks",
...
text(-161, 75.5, cex=0.53, bquote(paste(bolditalic(" "<= "6 Weeks"))))
text(-150, 53.5, cex=0.53, bquote(paste(bolditalic("> 6 to "<="12 Weeks"))))
text(-158.4, 25.5, cex=0.53, bquote(paste(bolditalic("> 12 Weeks"))))

text(-104.5, -2, cex=0.48, "Test for Subgroup Differences: Q = 2.05, df = 2, p = 0.36")
