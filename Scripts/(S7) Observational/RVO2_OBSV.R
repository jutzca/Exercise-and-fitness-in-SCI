...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_RVO2OBSV, ylim = c(-2, 11), xlim = c(-40, 20),at = c(-6, -3, 0, 3, 6), rows = c(7:7, 3:2), cex=0.4, digits = c(1,0),
...
addpoly(REHAB_REM_RVO2OBSV, row=5.5, cex=0.4, col=11, mlab="")
text(-26.4, 5.5, cex=0.4, bquote(paste("RE Model for Inpatient Rehabilitation: p = 0.94",
                                       "; ", I^2, " = ", .(formatC(REHAB_REM_RVO2OBSV$I2, digits = 1, format="f")), "%")))

addpoly(FREE_REM_RVO2OBSV, row=0.5, cex=0.4, col=11, mlab="")
text(-28.8, 0.7, cex=0.4, bquote(paste("RE Model for Free-Living: p = 0.06",
                                       "; ", I^2, " = ", .(formatC(FREE_REM_RVO2OBSV$I2, digits = 1, format="f")), "%")))

text(c(-28, -24, -20, -16, -12), 9.5, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-26, -18), 10.5, c('Pre', 'Post'), cex=0.3)
text(16, 9.5, cex=0.4, bquote(paste(bold("MD [95% CI]"))))
text(-37.1, 9.5, cex=0.4, bquote(paste(bold("Study"))))
text(10, 9.5, cex=0.4, bquote(paste(bold("Weight"))))

text(-22.5, -0.95, cex=0.4, bquote(paste("for All Studies: p < 0.001", 
...
text(-33.2, 8.2, cex=0.44, bquote(paste(bolditalic("Inpatient Rehabilitation"))))
text(-35.9, 4.2, cex=0.44, bquote(paste(bolditalic("Free-Living"))))

text(-26.3, -1.8, cex=0.4, "Test for Subgroup Differences: Q = 2.04, df = 1, p = 0.15")
