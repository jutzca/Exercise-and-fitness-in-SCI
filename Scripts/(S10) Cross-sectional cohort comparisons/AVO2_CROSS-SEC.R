...
# Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2CS, ylim = c(-2,14), xlim = c(-12.5, 7), alim = c(-2,2), rows = c(11:1), cex=0.4,
...
text(c(-8, -7, -6, -5, -4, -3), 12.5, c('Mean', 'SD', 'Total N', 'Mean', 'SD', 'Total N'), cex=0.38)
text(c(-7, -4), 13.5, c('Inactive', 'Active'), cex=0.38)
text(4.2, 12.5, cex=0.4, bquote(paste(bold("Weight"))))
text(6, 12.5, cex=0.4, bquote(paste(bold("MD [95% CI]"))))

text(-9.35, -0.94, cex=0.4, bquote(paste(": p < 0.001", 
                                        "; ",Z, " = ",.(formatC(REM_AVO2CS$zval, digits = 2, format="f")),
                                        "; ",I^2, " = ",.(formatC(REM_AVO2CS$I2, digits=1, format="f")), "%"))) 
text(0, -4, cex=0.4, bquote(paste(bold("(L/min)"))))

par(mar=c(4,4,1,2))
funnel(REM_AVO2CS, ylim = c(0,0.4), digits = c(2,1), cex = 0.5, xlab = 'Mean Difference (L/min)')

# Egger's test
regtest(REM_AVO2CS, model = "rma", ret.fit = FALSE, digits = 2)
