...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_RVO2CPET, ylim = c(-1, 89), xlim = c(-80, 60), at = c(-10, 0, 10, 20), rows = c(84:29, 24:2), digits = 1, cex=0.4,
...
addpoly(MATCH_REM_RVO2CPET, row=27.5, cex=0.4, col=11, mlab="")
text(-48, 27.5, cex=0.38, bquote(paste("RE Model for Matched CPET Modality: p < 0.001",
                                         "; ", I^2, " = ", .(formatC(MATCH_REM_RVO2CPET$I2, digits = 1, format="f")), "%")))

addpoly(UNMATCH_REM_RVO2CPET, row=0.5, cex=0.4, col=11, mlab="")
text(-48, 0.7, cex=0.38, bquote(paste("RE Model for Different CPET Modality: p < 0.001",
...
text(-40, -0.9, cex=0.38, bquote(paste("for All Studies (p < 0.001", 
...
text(-54, 85.5, cex=0.4, bquote(paste(bolditalic("CPET Modality Matches Intervention Modality"))))
text(-53.5, 25.5, cex=0.4, bquote(paste(bolditalic("CPET Modality Differs to Intervention Modality"))))

text(-50.2, -2, cex=0.38, "Test for Subgroup Differences: Q = 3.68, df = 1, p = 0.06")
