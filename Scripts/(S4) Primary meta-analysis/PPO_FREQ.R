...
PPOFREQ <- read.xlsx("PPO_DATA.xlsx", sheet = 8)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_PPOFREQ, ylim = c(-1, 79), xlim = c(-180, 135), at = c(-40, -20, 0, 20, 40, 60), rows = c(74:62, 57:20, 15:11, 6:2), digits = c(0,0),
...
addpoly(THREE_REM_PPOFREQ, row=60.5, cex=0.5, col=11, mlab="")
text(-107.5, 60.5, cex=0.5, bquote(paste("RE Model for < 3 Sessions/Week: p = 0.003",
                                       "; ", I^2, " = ", .(formatC(THREE_REM_PPOFREQ$I2, digits = 1, format="f")), "%")))

addpoly(THREEFIVE_REM_PPOFREQ, row=18.5, cex=0.5, col=11, mlab="")
text(-98.3, 18.5, cex=0.5, bquote(paste("RE Model for ">="3 to < 5 Sessions/Week: p < 0.004",
                                       "; ", I^2, " = ", .(formatC(THREEFIVE_REM_PPOFREQ$I2, digits = 1, format="f")), "%")))

addpoly(FIVE_REM_PPOFREQ, row=9.5, cex=0.5, col=11, mlab="")
text(-107.5, 9.5, cex=0.5, bquote(paste("RE Model for ">="5 Sessions/Week: p = 0.10",
                                       "; ", I^2, " = ", .(formatC(FIVE_REM_PPOFREQ$I2, digits = 1, format="f")), "%")))

addpoly(NR.CD_REM_PPOFREQ, row=0.5, cex=0.5, col=11, mlab="")
text(-122.7, 0.7, cex=0.5, bquote(paste("RE Model for NR/CD: p = 0.10",
                                       "; ", I^2, " = ", .(formatC(NR.CD_REM_PPOFREQ$I2, digits = 1, format="f")), "%")))
...
text(-149.1, 75.5, cex=0.5, bquote(paste(bolditalic("< 3 Sessions/Week"))))
text(-142.3, 58.5, cex=0.5, bquote(paste(bolditalic(" ">="3 to < 5 Sessions/Week"))))
text(-150.5, 16.5, cex=0.5, bquote(paste(bolditalic(" ">="5 Sessions/Week"))))
text(-133.3, 7.5, cex=0.5, bquote(paste(bolditalic("Not reported/Cannot determine"))))

text(-102, -2, cex=0.48, "Test for Subgroup Differences: Q = 23.88, df = 3, p < 0.001")

# Funnel plot
par(mar=c(4,4,1,2))

my_colours <- c("purple","blue","orange","red")[match(PPOFREQ$subgroup,c("three", "threefive", "five", "not reported/cannot determine"))]
