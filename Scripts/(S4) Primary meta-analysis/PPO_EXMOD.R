...
PPOEXMOD <- read.xlsx("PPO_DATA.xlsx", sheet = 4)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_PPOEXMOD, ylim = c(-1, 87), xlim = c(-180, 135), at = c(-40, -20, 0, 20, 40, 60), rows = c(82:57, 52:50, 45:32, 27:26, 21:10, 5:2), digits = c(0,0),
...
addpoly(AEROBIC_REM_PPOEXMOD, row=55.5, cex=0.5, col=11, mlab="")
text(-104, 55.5, cex=0.5, bquote(paste("RE Model for Aerobic, Upper-Body: p < 0.003",
                                       "; ", I^2, " = ", .(formatC(AEROBIC_REM_PPOEXMOD$I2, digits = 1, format="f")), "%")))

addpoly(RESISTANCE_REM_PPOEXMOD, row=48.5, cex=0.5, col=11, mlab="")
text(-117.8, 48.5, cex=0.5, bquote(paste("RE Model for Resistance: p < 0.003",
                                       "; ", I^2, " = ", .(formatC(RESISTANCE_REM_PPOEXMOD$I2, digits = 1, format="f")), "%")))

addpoly(FES_REM_PPOEXMOD, row=30.5, cex=0.5, col=11, mlab="")
text(-124, 30.5, cex=0.5, bquote(paste("RE Model for FES: p < 0.003",
                                       "; ", I^2, " = ", .(formatC(FES_REM_PPOEXMOD$I2, digits = 1, format="f")), "%")))

addpoly(GAIT_REM_PPOEXMOD, row=24.5, cex=0.5, col=11, mlab="")
text(-117.4, 24.5, cex=0.5, bquote(paste("RE Model for Gait Training: p = 0.54",
                                       "; ", I^2, " = ", .(formatC(GAIT_REM_PPOEXMOD$I2, digits = 1, format="f")), "%")))

addpoly(MIXED_REM_PPOEXMOD, row=8.5, cex=0.5, col=11, mlab="")
text(-122, 8.5, cex=0.5, bquote(paste("RE Model for Mixed: p < 0.003",
                                       "; ", I^2, " = ", .(formatC(MIXED_REM_PPOEXMOD$I2, digits = 1, format="f")), "%")))

addpoly(BEVC_REM_PPOEXMOD, row=0.5, cex=0.5, col=11, mlab="")
text(-108.5, 0.7, cex=0.5, bquote(paste("RE Model for Behaviour Change: p = 0.12",
...
text(-133, 83.5, cex=0.5, bquote(paste(bolditalic("Aerobic, Volitional Upper-Body"))))
text(-148, 53.5, cex=0.5, bquote(paste(bolditalic("Resistance Training"))))
text(-131, 46.5, cex=0.5, bquote(paste(bolditalic("Functional Electrical Stimulation"))))
text(-157, 28.5, cex=0.5, bquote(paste(bolditalic("Gait Training"))))
text(-140, 22.5, cex=0.5, bquote(paste(bolditalic("Hybrid, Mixed, Multimodal"))))
text(-149.7, 6.5, cex=0.5, bquote(paste(bolditalic("Behaviour Change"))))

text(-102, -2, cex=0.48, "Test for Subgroup Differences: Q = 18.30, df = 5, p = 0.003")
