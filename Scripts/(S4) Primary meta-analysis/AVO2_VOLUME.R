...
AVO2VOLUME <- read.xlsx("AVO2_DATA.xlsx", sheet = 9)
...
SGDIFF <- metagen(AVO2VOLUME_MD, AVO2SEI, studlab=AVO2VOLUME$study, data=AVO2VOLUME_ESCALC, fixed=FALSE, method.tau="REML", subgroup=subgroup)
SGDIFF

## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2VOLUME, ylim = c(-1,87), xlim = c(-11.5,7), alim = c(-2,2), rows = c(82:70, 65:36, 31:19, 14:2), cex = 0.4,
...
addpoly(FITNESS_REM_AVO2VOLUME, row=68.5, cex=0.4, col=11, mlab="")
text(-7.8, 68.5, cex=0.4, bquote(paste("RE Model for 40-89 Minutes/Week: p < 0.001",
                                        "; ", I^2, " = ", .(formatC(FITNESS_REM_AVO2VOLUME$I2, digits = 1, format="f")), "%")))

addpoly(CMETAB_REM_AVO2VOLUME, row=34.5, cex=0.4, col=11, mlab="")
text(-7.65, 34.5, cex=0.4, bquote(paste("RE Model for 90-149 Minutes/Week: p < 0.001",
                                        "; ", I^2, " = ", .(formatC(CMETAB_REM_AVO2VOLUME$I2, digits = 1, format="f")), "%")))

addpoly(GENPOP_REM_AVO2VOLUME, row=17.5, cex=0.4, col=11, mlab="")
text(-7.8, 17.5, cex=0.4, bquote(paste("RE Model for " >="150 Minutes/Week: p < 0.001",
                                        "; ", I^2, " = ", .(formatC(GENPOP_REM_AVO2VOLUME$I2, digits = 1, format="f")), "%")))

addpoly(NR.CD_REM_AVO2VOLUME, row=0.5, cex=0.4, col=11, mlab="")
text(-8.59, 0.7, cex=0.4, bquote(paste("RE Model for NR/CD: p < 0.001",
                                        "; ", I^2, " = ", .(formatC(NR.CD_REM_AVO2VOLUME$I2, digits = 1, format="f")), "%")))
...
text(-7.3, 83.5, cex=0.44, bquote(paste(bolditalic("SCI-Specific Exercise Guidelines for Fitness (40-89 Min/Wk)"))))
text(-6.2, 66.5, cex=0.44, bquote(paste(bolditalic("SCI-Specific Exercise Guidelines for Cardiometabolic Health (90-149 Min/Wk)"))))
text(-6.9, 32.5, cex=0.44, bquote(paste(bolditalic("Achieving General Population Exercise Guidelines (">= "150 Min/Wk)"))))
text(-9.16, 15.5, cex=0.44, bquote(paste(bolditalic("Not reported/Cannot determine"))))

text(-7.75, -2, cex=0.4, "Test for Subgroup Differences: Q = 1.55, df = 3, p = 0.67")

# Funnel plot
par(mar=c(4,4,1,2))

my_colours <- c("purple","blue","orange","red")[match(AVO2VOLUME$subgroup,c("fitness", "cmetab", "genpop", "not reported/cannot determine"))]
