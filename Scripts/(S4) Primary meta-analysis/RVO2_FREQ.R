...
RVO2FREQ <- read.xlsx("RVO2_DATA.xlsx", sheet = 8)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_RVO2FREQ, ylim = c(-1, 92), xlim = c(-85, 60), at = c(-10, 0, 10, 20), rows = c(87:75, 70:22, 17:9, 4:2), digits = 1,
...
addpoly(THREE_REM_RVO2FREQ, row=73.5, cex=0.4, col=11, mlab="")
text(-57, 73.5, cex=0.38, bquote(paste("RE Model for <3 Sessions/Week: p < 0.002",
                                       "; ", I^2, " = ", .(formatC(THREE_REM_RVO2FREQ$I2, digits = 1, format="f")), "%")))

addpoly(THREEFIVE_REM_RVO2FREQ, row=20.5, cex=0.4, col=11, mlab="")
text(-53, 20.5, cex=0.38, bquote(paste("RE Model for ">="3 to < 5 Sessions/Week: p < 0.002",
                                       "; ", I^2, " = ", .(formatC(THREEFIVE_REM_RVO2FREQ$I2, digits = 1, format="f")), "%")))

addpoly(FIVE_REM_RVO2FREQ, row=7.5, cex=0.4, col=11, mlab="")
text(-56, 7.5, cex=0.38, bquote(paste("RE Model for ">="5 Sessions/Week: p < 0.002",
                                       "; ", I^2, " = ", .(formatC(FIVE_REM_RVO2FREQ$I2, digits = 1, format="f")), "%")))

addpoly(NR.CD_REM_RVO2FREQ, row=0.5, cex=0.4, col=11, mlab="")
text(-62.4, 0.5, cex=0.38, bquote(paste("RE Model for NR/CD: p < 0.001",
                                       "; ", I^2, " = ", .(formatC(NR.CD_REM_RVO2FREQ$I2, digits = 1, format="f")), "%")))
...
text(-72.5, 88.5, cex=0.44, bquote(paste(bolditalic("< 3 Sessions/Week"))))
text(-69.9, 71.5, cex=0.44, bquote(paste(bolditalic(" ">="3 to < 5 Sessions/Week"))))
text(-73.2, 18.5, cex=0.44, bquote(paste(bolditalic(" ">="5 Sessions/Week"))))
text(-66.5, 5.5, cex=0.44, bquote(paste(bolditalic("Not reported/Cannot determine"))))

text(-55.7, -2, cex=0.38, "Test for Subgroup Differences: Q = 4.25, df = 3, p = 0.24")

# Funnel plot
par(mar=c(4,4,1,2))

my_colours <- c("purple","blue","orange","red")[match(RVO2FREQ$subgroup,c("three", "threefive", "five", "not reported/cannot determine"))]
