...
AVO2EXMOD <- read.xlsx("AVO2_DATA.xlsx", sheet = 4)
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2EXMOD, ylim = c(-1,95), xlim = c(-11.5,7), alim = c(-2,2), rows = c(90:66, 61:58, 53:37, 32:23, 18:9, 4:2),
...
addpoly(AEROBIC_REM_AVO2EXMOD, row=64.5, cex=0.4, col=11, mlab="")
text(-7.72, 64.5, cex=0.4, bquote(paste("RE Model for Aerobic, Upper-Body: p < 0.004",
                                     "; ", I^2, " = ", .(formatC(AEROBIC_REM_AVO2EXMOD$I2, digits = 1, format="f")), "%")))

addpoly(RESISTANCE_REM_AVO2EXMOD, row=56.5, cex=0.4, col=11, mlab="")
text(-8.4, 56.5, cex=0.4, bquote(paste("RE Model for Resistance: p = 0.003",
                                     "; ", I^2, " = ", .(formatC(RESISTANCE_REM_AVO2EXMOD$I2, digits = 1, format="f")), "%")))

addpoly(FES_REM_AVO2EXMOD, row=35.5, cex=0.4, col=11, mlab="")
text(-8.7, 35.5, cex=0.4, bquote(paste("RE Model for FES: p < 0.004",
                                     "; ", I^2, " = ", .(formatC(FES_REM_AVO2EXMOD$I2, digits = 1, format="f")), "%")))

addpoly(GAIT_REM_AVO2EXMOD, row=21.5, cex=0.4, col=11, mlab="")
text(-8.38, 21.5, cex=0.4, bquote(paste("RE Model for Gait Training: p = 0.14",
...
text(c(-7, -6, -5, -4, -3), 94, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-6.5, -4.5), 95, c('Pre', 'Post'), cex=0.3)
text(3.9, 94, cex=0.4, bquote(paste(bold("Weight"))))
...
text(-9.12, 91.5, cex=0.44, bquote(paste(bolditalic("Aerobic, Volitional Upper-Body"))))
text(-9.87, 62.5, cex=0.44, bquote(paste(bolditalic("Resistance Training"))))
text(-9.08, 54.5, cex=0.44, bquote(paste(bolditalic("Functional Electrical Stimulation"))))
text(-10.32, 33.5, cex=0.44, bquote(paste(bolditalic("Gait Training"))))
text(-9.49, 19.5, cex=0.44, bquote(paste(bolditalic("Hybrid, Mixed, Multimodal"))))
text(-9.95, 5.5, cex=0.44, bquote(paste(bolditalic("Behaviour Change"))))

text(-7.7, -2, cex=0.4, "Test for Subgroup Differences: Q = 10.11, df = 5, p = 0.07")
