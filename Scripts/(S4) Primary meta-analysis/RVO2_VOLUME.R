...
RVO2VOLUME <- read.xlsx("RVO2_DATA.xlsx")
...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_RVO2VOLUME, ylim = c(-1, 92), xlim = c(-85, 60), at = c(-10, 0, 10, 20), rows = c(87:74, 69:39, 34:14, 9:2), digits = 1,
...
addpoly(FITNESS_REM_RVO2VOLUME, row=72.5, cex=0.4, col=11, mlab="")
text(-55.8, 72.5, cex=0.38, bquote(paste("RE Model for 40-89 Minutes/Week: p < 0.002",
                                         "; ", I^2, " = ", .(formatC(FITNESS_REM_RVO2VOLUME$I2, digits = 1, format="f")), "%")))


addpoly(CMETAB_REM_RVO2VOLUME, row=37.5, cex=0.4, col=11, mlab="")
text(-55, 37.5, cex=0.38, bquote(paste("RE Model for 90-149 Minutes/Week: p < 0.002",
                                         "; ", I^2, " = ", .(formatC(CMETAB_REM_RVO2VOLUME$I2, digits = 1, format="f")), "%")))

addpoly(GENPOP_REM_RVO2VOLUME, row=12.5, cex=0.4, col=11, mlab="")
text(-55.5, 12.5, cex=0.38, bquote(paste("RE Model for " >="150 Minutes/Week: p < 0.002",
                                        "; ", I^2, " = ", .(formatC(GENPOP_REM_RVO2VOLUME$I2, digits = 1, format="f")), "%")))

addpoly(NR.CD_REM_RVO2VOLUME, row=0.5, cex=0.4, col=11, mlab="")
text(-62.8, 0.5, cex=0.38, bquote(paste("RE Model for NR/CD: p = 0.003",
                                         "; ", I^2, " = ", .(formatC(NR.CD_REM_RVO2VOLUME$I2, digits = 1, format="f")), "%")))
...
text(-51, 88.5, cex=0.4, bquote(paste(bolditalic("SCI-Specific Exercise Guidelines for Fitness (40-89 Min/Week)"))))
text(-42.5, 70.5, cex=0.4, bquote(paste(bolditalic("SCI-Specific Exercise Guidelines for Cardiometabolic Health (90-149 Min/Week)"))))
text(-48.3, 35.5, cex=0.4, bquote(paste(bolditalic("Achieving General Population Exercise Guidelines (">= "150 Min/Week)"))))
text(-66.8, 10.5, cex=0.4, bquote(paste(bolditalic("Not reported/Cannot determine"))))

text(-55.7, -2, cex=0.38, "Test for Subgroup Differences: Q = 1.44, df = 3, p = 0.70")

# Funnel plot
par(mar=c(4,4,1,2))

my_colours <- c("purple","blue","orange","red")[match(RVO2VOLUME$subgroup,c("fitness", "cmetab", "genpop", "not reported/cannot determine"))]
