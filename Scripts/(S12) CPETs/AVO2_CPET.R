...
## Forest plot
par(mar=c(4,4,0,2))
forest(REM_AVO2CPET, ylim = c(-1,79), xlim = c(-11.5,7), alim = c(-2,2), rows = c(74:22, 17:2), cex=0.4,
...
addpoly(MATCH_REM_AVO2CPET, row=20.5, cex=0.4, col=11, mlab="")
text(-8.07, 20.5, cex=0.4, bquote(paste("RE Model for matched CPET: p < 0.001",
                                       "; ", I^2, " = ", .(formatC(MATCH_REM_AVO2CPET$I2, digits = 1, format="f")), "%")))

addpoly(UNMATCH_REM_AVO2CPET, row=0.5, cex=0.4, col=11, mlab="")
text(-8.1, 0.7, cex=0.4, bquote(paste("RE Model for different CPET: p = 0.005",
                                       "; ", I^2, " = ", .(formatC(UNMATCH_REM_AVO2CPET$I2, digits = 1, format="f")), "%")))

text(c(-7, -6, -5, -4, -3), 78, c('Mean', 'SD', 'Mean', 'SD', 'Total N'), cex=0.3)
text(c(-6.5, -4.5), 79, c('Pre', 'Post'), cex=0.3)
text(3.9, 78, cex=0.4, bquote(paste(bold("Weight"))))
...
text(-8.2, 75.5, cex=0.44, bquote(paste(bolditalic("CPET Modality Matches Intervention Modality"))))
text(-8.18, 18.5, cex=0.44, bquote(paste(bolditalic("CPET Modality Differs to Intervention Modality"))))

text(-7.75, -2, cex=0.4, "Test for Subgroup Differences: Q = 4.23, df = 1, p = 0.04")
